...
| missing image | `caterpillar` | Inspired by the classic game "Centipede". The player must destroy an approaching line of space ships before they reach the bottom of the screen. When a space ship is destroyed it leaves a wall behind. The player cannot leave the bottom part of the screen.
| missing image | `gemjourney` | Inspired by the classic game "Crystal Quest". Birds have stolen your precious pig gems. They will defend the stolen gems at all costs and have riddled the area with deadly crates crafted from the wood of the pigsbane tree. Fortunately you've brought an arsenal of foxes.
| missing image | `safezone` | missing description
| missing image | `hovercraft` | missing description
